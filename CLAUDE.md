...
## If you are being asked to INSTALL this, not work on it

Follow **`SETUP.md`**, top to bottom. It is written to be executed.

Three things only the human can provide, so ask for them rather than inventing
them: a Firefly email and password to register (**16 characters minimum** —
Firefly's own rule, and it rejects shorter ones by answering 200 and
re-rendering the form), a bank statement to put in `inbox/`, and a password for
passbook's own web UI.

Everything else — Docker, Python, secrets, the stack, the API token, the
currency, the asset account and its opening balance — `make setup` does. Do not
hand-write `.env`, and do not create the Firefly asset account by hand: its
opening balance has to be the statement's *opening* balance, and the wizard
reads that out of the file.

**Docker:** run `make preflight` first. It diagnoses what Docker needs on *this*
machine and names the one command that is right here — which is not the same
command everywhere, and the wrong one fails in a way that reads like a broken
install. `make setup` will offer to run the fix; let it ask the human rather than
running `sudo` yourself. If the fix is `usermod -aG docker`, the session has to
be restarted before anything else will work — say so and stop.

## Environment facts

- **Two supported routes on every platform, and both are documented per OS** in
  `docs/install-{linux,windows,macos}.md`: Docker Desktop for a GUI, Docker
  Engine (or Colima on macOS) for a terminal. Nothing in the stack changes
  between them.
- Docker Desktop is what a first-time user is pointed at, because it is one
  installer and manages the WSL2 backend itself. **Docker Engine inside WSL2 is
  the author's own setup** (SPEC D8) and is a first-class path, not a footnote —
  it just costs four manual steps, each of which fails quietly in its own way.
- **Colima on macOS is untested by this project** and says so where it is
  documented. "No reason it should not work" is not a measurement.
...
| A rejected Firefly registration answers 4xx | It answers **200 and re-renders the form**. The password minimum is **16**, not the 8 the wizard first asked for, so a status-code check reports success and then cannot log in (§22.4). |
| A workflow's `GITHUB_TOKEN` can read the repo's own traffic API | **403**, even with `contents: write`. The endpoint needs `administration: read`, which is not a scope a workflow token can be granted. Measured on run 32881543735. The traffic badges it was for were removed as misleading (§22.8), but the token fact stands. |
| `make setup` is the install command everywhere | **Windows ships no `make`.** SETUP.md told every user to run it, which is a wall for the largest desktop audience. `python scripts\setup.py` and `launchers\start-passbook.cmd` are the Windows paths (§22.4). |
| `systemctl` on PATH means systemd is running | WSL ships the binary regardless. Measured here: `is-system-running` says `offline` and PID 1 is `init(Ubuntu)`, so `systemctl start docker` fails with an error about systemd rather than about Docker. `service docker start` is the right command (§22.4). |
