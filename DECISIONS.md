...
> the licence question is settled as *none for now*, with the Firefly III
> analysis recorded because it outlives the choice (§22.3); the
...
### 22.3 Licence — none, for now, deliberately

**passbook ships under no open-source licence.** Default copyright applies: all
rights reserved. This is the operator's decision, recorded here rather than left
as an absence somebody later mistakes for an oversight.

**What that costs, stated plainly.** Without a licence nobody may legally copy,
modify, fork or redistribute the code — reading it is all that is permitted. For
a repository whose README invites people to add their own bank, that is a real
tension, and it is a tension the operator has chosen to accept for now. The
`Adding a bank` guide and the contribution templates remain, because the
mechanism is sound; what is missing is the permission, and CONTRIBUTING.md now
says so before anyone spends an afternoon on a pull request.

Reversing it is one file and three edits. If it is reversed, the analysis below
is the part that still matters.

#### Does Firefly III's AGPL reach us? No — and this is why it never did

Answered by reading the licence rather than recalling it, and it is unchanged by
the decision above.
...
program. **Firefly's licence does not constrain this project's own terms.** The
one change that would alter that: vendoring or patching Firefly's source, at
which point the obligation attaches to *that patched Firefly*.

#### What the dependencies allow

Checked rather than assumed — every installed distribution's own metadata was
read. All are permissive (MIT, BSD, Apache-2.0, MIT-CMU, ZPL 2.1) except
`pikepdf`, which is MPL-2.0: weak copyleft that reaches only MPL-licensed files,
not the program using them. **Nothing in the dependency tree constrains the
choice**, so any licence — including none — remains available.

#### The fonts are a real obligation, and they are not optional

The three redistributed families are **OFL-1.1**, and that licence requires its
text to travel with the fonts however the surrounding project is licensed. The
subsets ship with the full licence text and all three copyright lines in
`frontend/src/fonts/OFL.txt`, beside the fonts themselves, which is where the
OFL wants it. `NOTICE.md` carried a duplicate of this and was removed; the file
next to the fonts is the one that discharges the obligation. Subsetting is a
Modified Version
under clause 2 and is permitted; the family names are unchanged, which clause 5
allows because it forbids renaming a derivative *to* a reserved name, not
distributing a subset under it.

**This is the one licensing thing that cannot be dropped along with the rest.**

### 22.4 Setup for someone who is not a developer

**Both Docker routes are first-class**, in one `SETUP.md` covering all three
operating systems — Docker Desktop for a GUI, Docker Engine (or Colima on macOS)
for a terminal, the latter behind a `<details>` so it does not tax someone who
does not want it. Nothing in the stack changes between them, which is what makes
carrying both cheap.

They were three separate per-OS pages for one revision, and that was wrong: it
put four setup files in front of a new user, who then has to choose before they
have learned anything. One page with a table and a fold is less to read and less
to maintain.

D8 — Docker Engine inside WSL2 — remains the author's own configuration and is
written up in full on the Windows page. It costs four manual steps (systemd in
`/etc/wsl.conf`, a `wsl --shutdown`, a package install, a group change with a
re-login), each of which fails quietly in its own way. That is the reason it is
not the default, not a judgement about the tool.

Colima is documented as the macOS terminal route **and labelled untested**, which
is the honest state: the stack is four ordinary containers with no privileged
access, no host networking and no bind mounts outside the project, so there is no
reason it should fail — and "no reason it should" is not a measurement.
...
had a token in it since Phase 3. Both `make up` and the wizard now start in two
stages.

#### The wizard asks for four things, not nine

An earlier version walked the user through registering in a browser, finding the
token page, creating a token, pasting it, setting the currency, and creating an
asset account with the right opening balance. Six manual steps, two of which are
this project's most expensive traps — the Command-line-token mix-up and
Firefly's invitation to enter your *current* balance.

All six are machine-doable, and are now done:

| Was asked | Now |
|---|---|
| register in a browser | `POST /register`, from the wizard |
| create and paste a token | `POST /oauth/personal-access-tokens`, after `passport:client --personal` |
| set the currency to INR | `POST /api/v1/currencies/INR/enable` then `/primary` |
| create an asset account | `POST /api/v1/accounts`, with the opening balance **read out of the statement** |
| type the account number | read out of the statement (§6.3) |

What remains is a Firefly email and password, a statement in `inbox/`, and a
password for passbook itself.

Reading the statement needs the parser, and a fresh clone has nothing installed —
so the wizard installs the dependencies first (`uv sync`, or a plain venv if uv
is absent) and runs the parse in that interpreter rather than in the bare system
python it was launched with. Without that it would have degraded to the
"ask them" path on precisely the machine where the automation matters most: the
new one.

**What is pinned and what is not, stated because it decides the failure mode.**
The currency and the account come from the REST API this project pins and reads
from source. Registration and token minting do not: `POST /register` is a browser
form and the token endpoint is Passport's own. Those are likelier to shift
between Firefly versions, so provisioning is **best-effort by construction** —
anything unexpected falls back to the manual prompts, which is exactly what the
wizard did before. A Firefly change degrades this to the old flow rather than to
a dead end.

#### A measured trap, found by running it

The first version of the prompt asked for a password of **8+ characters**.
Firefly requires **16**, and it rejects a shorter one by answering **200 and
re-rendering the form** — no 4xx, no JSON error. A wizard that checked the status
code would have reported success and then failed to log in, and every user would
have been dumped into the manual fallback with no explanation.
...
"Start the Docker service" is **four different commands**, and the wrong one
fails in a way that reads like a broken install:

| Situation | Right command |
|---|---|
| Linux with systemd | `sudo systemctl start docker` |
| WSL2 without systemd | `sudo service docker start` |
| Docker Desktop | open the app — there is no service |
| Installed, no permission | `sudo usermod -aG docker $USER`, **then a fresh login** |

`scripts/dockercheck.py` decides which. It is the shared half of `preflight`
(which reports) and `setup` (which offers to run it), and it distinguishes the
four cases rather than printing all of them.

**Measured, and this is the row that justifies the module.** On the machine this
was written on `systemctl` is on `PATH`, `systemctl is-system-running` answers
`offline`, and PID 1 is `init(Ubuntu)` — WSL2 without `[boot] systemd=true`.
Treating "the binary exists" as "systemd works" would send the user to a command
that fails with *"System has not been booted with systemd"*, an error about
something other than their problem. So the check asks whether systemd is
**running**, with PID 1 as the second opinion.

Two more distinctions that matter: a permission problem is not reported as a dead
daemon (restarting a running daemon fixes nothing), and under WSL the offered
Docker Desktop link is the **Windows** installer — `platform.system()` says
`Linux` there, which is precisely the trap.

**Nothing is installed silently.** Every command is printed and confirmed before
it runs, anything needing `sudo` says so, and a fix that requires a re-login
stops the wizard and says why rather than continuing into a failure. Installing
system packages on someone's machine without asking is not a convenience.
...
* **Three launchers in `launchers/`**, one per OS, double-clickable, each
  deferring to `scripts/launch.py`. They are not redundant with `make setup`:
  **Windows ships no `make`**, so on the largest desktop platform the launcher
  (or `python scripts\setup.py`) is the only path there is — SETUP.md said
  `make setup` to everyone until that was noticed. They also carry the weekly
  re-start, which is the case a terminal user covers with `make up`.
  Moved out of the repository root because four files there were the first
  thing a visitor saw — which runs the wizard if the install is unconfigured and
...
* **The default port path is exercised.** The operator's own instance runs
  Firefly on a non-default host port because a Windows service holds 8080 (§5),
  and nothing had run end to end on the default since. Preflight checks 8080 and
  8081, reports a held port as a warning rather than a failure, and names the WSL
  mirrored-networking trap where `ss` inside the distro shows nothing at all.
...
Every install page says to clone. The reason is not preference: `git pull` is how
updates arrive, and `make upgrade` reads the repository to know which migrations
this install has already applied (§22.2). A ZIP download is a dead end that has
to be re-downloaded and reconfigured, and it silently opts the user out of the
one mechanism that keeps a pulled migration from breaking their ledger.
...
`CONTRIBUTING.md`, issue templates for bug,
...
### 22.8 No usage counter — considered, built, removed

Forks and stars are public; clones and visitors are not, so a workflow was built
to read this repository's own traffic and publish it as badges. It worked, and it
was removed after one look at the number.

**It was not measuring what it appeared to measure.** The badge read 14 unique
cloners on a repository days old, and most of those clones were this project's
own CI and its author testing a fresh checkout. A figure that looks like evidence
of interest and is mostly self-traffic is precisely the shape §19 and §20 exist
to prevent — a stated number nobody checked, rendered as a fact.

Two smaller reasons it was not worth keeping: a workflow's built-in
`GITHUB_TOKEN` is refused **403** on `/traffic/clones` even with
`permissions: contents: write` (the endpoint needs `administration: read`, which
is not a grantable workflow scope — measured on run 32881543735), so it needed a
personal access token to work at all; and it added a nightly job, a committed
state file and two generated artefacts to maintain, for a number that was wrong.

**Stars and forks stay**, because GitHub computes them and they cannot be
overstated by our own tooling.

**And there is no honest way to count running installs.** passbook is a localhost
application whose entire proposition is that nothing leaves the machine.
Telemetry would trade the audience it is built for against a dashboard figure.
The right answer is to say so rather than to build one, which is what the README
now does.
...
- [x] The licence question settled and written down — **none for now, all
      rights reserved** — with the Firefly III analysis answered from the
      licence text on the pinned tag rather than from memory, because that part
      is true whatever terms this project later adopts (§22.3).
...
- **The full `make setup` path has not been run end to end on a clean machine.**
  Its new provisioning half *was* measured, against a scratch Firefly 6.6.6 on
  its own network: registration, `passport:client --personal`, token minting,
  token validation, expiry decode, `currencies/INR/enable` + `/primary`, and an
  asset account created from the fixture's own opening balance — all confirmed,
  plus the short-password rejection surfacing Firefly's message verbatim.
...
## 40. The rules that do not bend

**Moved here from `CONTRIBUTING.md`, where they were the first thing a stranger
read.** Every one of them is a scar, and every one is worth keeping — but a
contributing guide that opens with twenty numbered prohibitions is a guide
nobody finishes, and the person it was written for is whoever is changing this
code next, which is mostly me. They are the constitution, not the welcome mat.
...
## 41. Writing a migration

**Moved here from `CONTRIBUTING.md`.** Most people who open a pull request will
never write one, and seventy lines of reference in a welcome page is seventy
lines nobody reads.
...
The goal, in the operator's words: *"a friend doesnt need to call me to how to
use this and that"*. That is a sharper specification than "good UX", because it
names the failure — a phone call — and it can be checked by looking.

It failed, and the screenshot is what found it. On a fresh install, before
anyone had done anything, the first screen read:

> **BALANCE** unavailable
> Ledger unverified · ✗ No backup
> No charts: No account is registered. **Set the missing value in .env on the
> host, then reload.** They are drawn from the ledger itself, so the store has
> to answer.

Every line of that is accurate. All of it is wrong:

* nothing is broken;
* nothing needs editing, least of all `.env` on "the host";
* the one thing to do — upload a statement — is not mentioned.

**A person who has done nothing yet was being shown a diagnosis of having done
nothing.** The empty state was inherited from the error state because they
happen to produce the same missing values, and nobody had looked at it: the
fixture always has 93 rows, so the suite never saw this screen and neither did
any previous screenshot run.

It is a welcome now: three numbered steps, one button, and two disclosures —
why never to use an online converter, and what to do if the bank is not one of
the three that ship. A test asserts the branch exists and that the words
`.env`, `unavailable` and `unverified` do not appear in it.
