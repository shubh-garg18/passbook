...
def check_docker(report: Report) -> None:
    """Ask `dockercheck` what is wrong HERE, and print the fix for here.

    Not a generic "start the Docker service": that is four different commands
    depending on the machine, and the wrong one fails in a way that reads like a
    broken install. See `scripts/dockercheck.py`.
    """
    diagnosis = dockercheck.diagnose()
    if diagnosis.ok:
        flavour = {"desktop": "Docker Desktop", "engine": "Docker Engine"}.get(
            diagnosis.kind, "Docker"
        )
        report.ok(f"{flavour} {diagnosis.version}")
        code, out = run("docker", "compose", "version", "--short")
        report.ok(f"docker compose {out}" if code == 0 else "docker compose")
        return

    lines = []
    for fix in diagnosis.fixes:
        lines.append(fix.label + ":")
        if fix.command:
            lines.append("  " + " ".join(fix.command))
        if fix.url:
            lines.append("  " + fix.url)
        if fix.note:
            lines.extend("  " + part for part in fix.note.split("\n"))
        if fix.then_relogin:
            lines.append("  then log out and back in — group changes need a new session")
    lines.append("")
    lines.append("`make setup` offers to run the runnable ones for you.")
    report.fail(diagnosis.problem, "\n".join(lines))
