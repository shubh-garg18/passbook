...
# Fixed, and safe to write down: this stack is a throwaway on a random loopback
# port holding nothing but the fixture, and it is destroyed by --down. Fixed
# rather than generated so the maintainer can sign in and photograph Firefly's
# own UI, which is half of what the README shows.
#
# 16+ characters because that is Firefly's own minimum, measured — a shorter one
# is refused by a form that answers 200 and simply re-renders (SPEC §22.4).
DEMO_EMAIL = "demo@example.com"
DEMO_PASSWORD = "demo-passbook-screenshot-stack"
...
def shoot_firefly(tag: str) -> None:
    """Photograph Firefly's OWN interface, holding the fixture.

    The README shows both halves — passbook's ledger page and the Firefly it
    writes into — because "self-hosted Firefly III" means nothing to someone who
    has never seen one. Same rule as every other published screenshot: the only
    data in frame came from `tests/fixtures/statement.xls`.
    """
    ...
    try:
        ...
        print("  (skipping the Firefly shot: playwright is not installed)")
        return
    ...
    base = f"http://localhost:{PORT}"
    print("  photographing Firefly itself")
    with sync_playwright() as p:
        ...
        for theme in ("light",):
            ...
            # The transaction list is the view that shows the rows arriving,
            # which is the thing worth showing.
            page.goto(f"{base}/transactions/withdrawal", wait_until="networkidle")
            page.wait_for_timeout(2500)
            page.screenshot(path=str(out / f"firefly-{theme}-desktop.png"), full_page=True)
            ctx.close()
        browser.close()
    print(f"  wrote {out / 'firefly-light-desktop.png'}")
